...
| Repository 抽象/开发期 JSON 存储 | `src/database/jsonStore.js` | 每个实体独立 JSON 文件，接口可替换为 MySQL 实现。 |
| 种子数据 | `src/database/seed.js` | 演示账号、游戏库、初始组局和场地。 |
| 用户/认证/权限 | `src/services/userService.js` | 登录、个人资料、实名认证审核、账号状态。 |
...
| 数据库设计 | `database/schema.sql` | MySQL 表、索引、外键和关键约束。 |
