...
\begin{enumerate}
  ...
  \item 在课程周期内完成可运行原型，同时预留 MySQL、学校统一身份认证、微信小程序和通知推送等后续扩展点。
\end{enumerate}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{P{2.7cm} P{3.2cm} P{5.2cm} Y}
    ...
    入口操作 & 主服务 & 协作服务 & 数据实体 \\
    \midrule
    发布组局 & SessionService & GameLibService、NotificationService、LogService & game\_sessions、session\_members、notifications、admin\_logs \\
    申请加入 & ApplicationService & SessionService、CreditService、NotificationService & session\_applications、session\_members、credit\_records \\
...
\begin{longtable}{P{1.5cm} P{5.0cm} P{3.0cm} P{6.0cm}}
  \toprule
  方法 & 路径 & 角色 & 说明 \\
  \midrule
  \endfirsthead
  ...
  GET/PUT & \texttt{/api/users/me} & 登录用户 & 查看或维护个人资料 \\
  GET & \texttt{/api/users/me/credit} & 登录用户 & 查看个人信用分与信用流水 \\
  GET/POST & \texttt{/api/games} & 公开/管理员 & 查询游戏库；管理员新增游戏 \\
  GET/POST & \texttt{/api/sessions} & 公开/认证学生 & 筛选组局；发布组局 \\
  GET/PATCH & \texttt{/api/sessions/:id} & 公开/发起人 & 查看详情；发起人编辑组局 \\
  POST & \texttt{/api/sessions/:id/applications} & 认证学生 & 申请加入组局或直接加入 \\
  ...
  GET/POST/PATCH & \texttt{/api/complaints} & 学生/管理员 & 提交、查询、处理投诉 \\
  GET/POST/PATCH & \texttt{/api/venue-reservations} & 发起人/场地管理员 & 提交、查询、审核场地预约 \\
  GET/PATCH & \texttt{/api/notifications} & 登录用户 & 查看通知与标记已读 \\
...
\resultfigure{er_diagram.png}{数据库 E-R 图}{fig:er}

数据库以 users 为主体，game\_sessions 为活动核心。报名申请、成员、预约、评价、投诉、信用、通知、日志均通过外键关联到用户或组局，便于追溯完整业务链路。
...
\begin{longtable}{P{3.2cm} P{5.4cm} P{4.0cm} P{3.0cm}}
  ...
  \midrule
  \endhead
  users & id, student\_no, name, nickname, role, auth\_status, credit\_score, status, tags, contact & PK id；UNIQUE student\_no & 用户、管理员与场地管理员统一建模，通过 role 区分权限。 \\
  ...
  game\_sessions & id, host\_id, game\_id, title, start\_time, end\_time, location, max\_members, current\_members, join\_mode, status & PK id；FK host\_id；FK game\_id & 一次具体组局活动。 \\
  ...
  session\_members & id, session\_id, user\_id, member\_role, join\_time, checkin\_status & PK id；FK session\_id；FK user\_id；UNIQUE(session\_id,user\_id) & 已加入成员名单。 \\
...
\subsection{关键索引与一致性设计}

\begin{enumerate}
  \item \texttt{game\_sessions} 建立 \texttt{(status, start\_time)}、\texttt{(game\_id, start\_time)} 和 \texttt{host\_id} 索引，支撑列表筛选和个人组局查询。
  \item \texttt{session\_members} 对 \texttt{(session\_id, user\_id)} 建唯一约束，防止重复报名；\texttt{session\_applications} 对待审核状态建立组合索引。
  \item \texttt{venue\_reservations} 对 \texttt{(venue\_id, start\_time, end\_time, status)} 建索引，审核时快速发现时间冲突。
  \item 报名审批、退出扣分、投诉成立后的信用变更应放在同一事务中执行；原型由单进程同步写文件保证演示一致性，生产由 MySQL 事务保证。
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Repository 抽象保证原型 JSON 存储和生产 MySQL 存储可以替换，接口和服务层不变。
\end{enumerate}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{P{3.0cm} P{5.4cm} Y}
    ...
    报告模块 & 代码实现 & 验收方式 \\
    \midrule
    接口/控制层 & \texttt{src/server.js}, \texttt{src/router.js} & 访问 \texttt{/api/health}，使用前端或测试脚本调用核心接口。 \\
    业务服务层 & \texttt{src/services/sessionService.js}, \texttt{venueService.js}, \texttt{complaintService.js} 等 & 通过 \texttt{node --test} 覆盖报名、审核、场地冲突、投诉信用变更。 \\
    数据访问层 & \texttt{src/database/jsonStore.js} 与 \texttt{data/*.json} & 重启后数据不丢失；删除 data 文件可重新种子初始化。 \\
    数据库设计 & \texttt{database/schema.sql} & 检查表、索引、外键与本报告字段一致。 \\
    前端演示 & \texttt{public/index.html}, \texttt{public/app.js}, \texttt{public/styles.css} & 浏览器打开本地服务即可完成学生/管理员/场地管理员流程。 \\
    \bottomrule
  \end{tabularx}
  \caption{设计与实现映射}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{P{3.0cm} P{6.0cm} Y}
    ...
    阶段 & 内容 & 完成标准 \\
    \midrule
    单元/接口测试 & 健康检查、登录、筛选、发布、申请、审核、退出、投诉、场地审核 & \texttt{npm test} 全部通过。 \\
